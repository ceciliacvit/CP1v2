...
controller.DesiredLinearVelocity = 0.12;
...
while drive
    ...
    [v, omega, lookaheadPt] = controller(currentPose);

    % Scale down linear velocity when heading error is large so the outer
    % wheel doesn't exceed its speed limit on the TurtleBot3.
    headingError = wrapToPi(atan2(lookaheadPt(2) - position(2), ...
                                  lookaheadPt(1) - position(1)) - angle);
    v = v * max(0, cos(headingError) .^ 8);
